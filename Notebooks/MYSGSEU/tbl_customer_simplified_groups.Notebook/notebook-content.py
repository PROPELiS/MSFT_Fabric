...
orderschema = StructType([
    ...
    StructField("CT_SimplifiedGroupId", StringType(), True),
...
if in_mode == "FULL":
    ...
    silver_table.alias("target").merge(
        ...
        "target.SimplifiedGroupId = source.SimplifiedGroupId"
    ...
    filtered_source_df = source_df_delta.filter(source_df_delta["SYS_CHANGE_OPERATION"] == "D") \
        .select("CT_SimplifiedGroupId").distinct()

    silver_table.alias("T") \
        .merge(filtered_source_df.alias("S"), "T.SimplifiedGroupId = S.CT_SimplifiedGroupId") \
        .whenMatchedDelete() \
        .execute()

    transformed_df = source_df_delta.alias("a").join(
        source_df_delta.groupBy("SimplifiedGroupId")
        .agg(spark_max(col("SYS_CHANGE_VERSION").cast("bigint")).alias("SYS_CHANGE_VERSION"))
        .alias("b"),
        (col("a.SimplifiedGroupId") == col("b.SimplifiedGroupId")) &
