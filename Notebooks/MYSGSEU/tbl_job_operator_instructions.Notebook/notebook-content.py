...
orderSchema = StructType([
    ...
    StructField("CT_InstructionId", StringType(), True),
	StructField("InstructionId", StringType(), True),
    StructField("InstructionBy", StringType(), True),
	StructField("JobTaskId", StringType(), True),
	StructField("InstructionDateTime", StringType(), True),
	StructField("JobStageId", StringType(), True),
    StructField("InstructionDetails", StringType(), True),
...
if in_mode == "FULL":
    ...
    silver_table.alias("target").merge(
           ...
           "target.InstructionId = source.InstructionId"
        ).whenMatchedUpdate(set={
        "SYS_CHANGE_VERSION": col("source.SYS_CHANGE_VERSION"),
        "SYS_CHANGE_OPERATION": col("source.SYS_CHANGE_OPERATION"),
        "InstructionId": col("source.InstructionId"),
        "InstructionBy": col("source.InstructionBy"),
        "JobTaskId": col("source.JobTaskId"),
        "InstructionDateTime": col("source.InstructionDateTime"),
		"JobStageId": col("source.JobStageId"),
        "InstructionDetails": col("source.InstructionDetails"),
    ...
    .select("CT_InstructionId") \
    .distinct()
 
   # Perform the MERGE operation
    silver_table.alias("T") \
    .merge(filtered_source_df.alias("S"), "T.InstructionId = S.CT_InstructionId") \
    .whenMatchedDelete() \
    .execute()
 
    # Perform the transformation with all required columns
    transformed_df = source_df_delta.alias("a").join(
        source_df_delta.groupBy("InstructionId")
        .agg(spark_max(col("SYS_CHANGE_VERSION").cast("bigint")).alias("SYS_CHANGE_VERSION"))
        .alias("b"),
        (col("a.InstructionId") == col("b.InstructionId")) & 
    ...
    .select(
        ...
        col("a.InstructionBy"),
        col("a.JobTaskId"),
        col("a.InstructionDateTime"),
		col("a.JobStageId"),
        col("a.InstructionDetails"),
		col("a.JobVersionId")
    ).distinct()
