...
if in_mode == "FULL":
    ...
    df.write.format("delta").mode("overwrite").saveAsTable("tbl_job_tech_spec_checklist_amends")
    bronze_Path ="abfss://SGSCo_Fabric_Development@onelake.dfs.fabric.microsoft.com/BRONZE.Lakehouse/Tables/MYSGSEU/tbl_job_tech_spec_checklist_amends_FULL"
    ...
    df.write.format("delta").mode("append").saveAsTable("tbl_job_tech_spec_checklist_amends")
    
    source_path = "abfss://SGSCo_Fabric_Development@onelake.dfs.fabric.microsoft.com/BRONZE.Lakehouse/Tables/MYSGSEU/tbl_job_tech_spec_checklist_amends_DELTA"
...
df = spark.sql("SELECT COUNT(*) FROM SILVER.dbo.tbl_job_tech_spec_checklist_amends")
display(df)
...
df = spark.sql("SELECT COUNT(*) FROM BRONZE.MYSGSEU.tbl_job_tech_spec_checklist_amends_FULL")
display(df)
...
df = spark.sql("SELECT * FROM BRONZE.MYSGSEU.tbl_job_tech_spec_checklist_amends_DELTA")
display(df)
