...
src_filtered AS (
  SELECT sd.*
  FROM src_dedup sd
  LEFT JOIN delta.`abfss://Propelis_Production@onelake.dfs.fabric.microsoft.com/BRONZE.Lakehouse/Tables/UKG/combined_paycode` cp
   ON TRIM(LOWER(sd.PayCodeName)) = TRIM(LOWER(cp.PayCodeName))
WHERE cp.PayCodeName IS NULL
)
