CREATE     PROCEDURE [Propelis].[Proc_EMPLOYEE_RESP_2]
AS
BEGIN
    ...
    SET
        ...
        T.[Employee Status Indicator]     = S.[EMPL_STATUS_IND],
    ...
    FROM [GLOBAL_EDW].[Propelis].[EMPLOYEE_RESP_2] AS T
    ...
    INSERT INTO [GLOBAL_EDW].[Propelis].[EMPLOYEE_RESP_2] (
        [PERSONEL_KEY], -- Auto-Fix: Added PERSONEL_KEY to ensure it is not NULL
        [Personnel Number],
        [Company Code],
        [Plant ID],
        [Cost Center ID],
    ...
    SELECT
        S.[PERSONEL_KEY], -- Auto-Fix: Ensure PERSONEL_KEY is selected to avoid NULL
        S.[PERSONEL_NUM],
        S.[CMPNY_CD],
        S.[PLNT_ID],
        S.[COST_CNTR_ID],
    ...
    LEFT JOIN [GLOBAL_EDW].[Propelis].[EMPLOYEE_RESP_2] AS T
        ON T.[PERSONEL_KEY] = S.[PERSONEL_KEY]
