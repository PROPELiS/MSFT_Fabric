CREATE   PROCEDURE [Propelis].[Proc_Partner_Profit_Center_Trading_Partner_Grouping]
AS
BEGIN
    ------------------------------------------------------------------
    ...
    SET 
        ...
        [Partner Trading Partner ID] = S.[TRADING_PRTNR]
    FROM [GLOBAL_EDW].[Propelis].[Partner_Profit_Center_Trading_Partner_Grouping] AS T
    INNER JOIN #SourceData AS S
        ON T.[PROFT_CNTR_ID] = S.[PROFT_CNTR_ID]
    WHERE HASHBYTES('SHA2_256', CONCAT(
            ISNULL(T.[PROFT_CNTR_ID], ''), '|',
            ISNULL(T.[Partner Trading Partner ID], '')
        )) <> S.HashKey;

    ------------------------------------------------------------------
    -- Step 3: Insert only new rows not already in target
    ------------------------------------------------------------------
    INSERT INTO [GLOBAL_EDW].[Propelis].[Partner_Profit_Center_Trading_Partner_Grouping]
    (
        [PROFT_CNTR_ID],
        [Partner Trading Partner ID]
    )
    SELECT 
        S.[PROFT_CNTR_ID],
        S.[TRADING_PRTNR]
