CREATE   PROCEDURE [Propelis].[Proc_RESP_COST_CENTER_VERTICAL_HIERARCHY]
AS
BEGIN
    ----------------------------------------------------------------------
    ...
    FROM [GLOBAL_EDW].[Propelis].[RESP_COST_CENTER_VERTICAL_HIERARCHY] T
    ...
    -- Insert new records
    ----------------------------------------------------------------------
    INSERT INTO [GLOBAL_EDW].[Propelis].[RESP_COST_CENTER_VERTICAL_HIERARCHY]
    (
        [COST_CNTR_KEY],
    ...
    LEFT JOIN [GLOBAL_EDW].[Propelis].[RESP_COST_CENTER_VERTICAL_HIERARCHY] T
        ON T.[COST_CNTR_KEY] = S.[COST_CNTR_KEY]
